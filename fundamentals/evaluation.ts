 ...
 * operand to run, resulting in "3". Now, there are no operands and it's a single value,
 * which we can't further evaluate/calculate, so we return it.
 *
 * The same way we evaluate a variable, we can evaluate a function (to which a 
 * variable can point). So we need to run the function and result in some kind
 * of primitive value like number, string, array
 *
...
//    return sum(1, 2); (we pass the value to which the variables point)
//
// The function sum(a, b) is called with the values 1 and 2 as it's arguments:
// a ---> 1;
// b ---> 2;
//
...
//    return a + b
//
// In order to use the + operand, we need to evaluate a and b:
// We evaluate a and b to 1 and 2:
//    return 1 + 2
//
...
// The previous return sum(a, b) is evaluated to 3 and we return it as a result.
//    return 3
